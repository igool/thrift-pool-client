...
import com.github.igool.thrift.client.utils.FailoverCheckingStrategy;
...
public class FailoverThriftClientImpl implements ThriftClient {
    ...
    private class FailoverStategy implements
        ...
        public List<ThriftServerInfo> get() {
            ...
            List<ThriftServerInfo> serviceInfoList = new ArrayList<ThriftServerInfo>();
            for(Iterator<ThriftServerInfo> it = originalServerInfoProvider.get().iterator();it.hasNext();){
            	ThriftServerInfo info = it.next();
            	if ( !failedServers.contains(info) ){
